...
static void check_split_2(struct kunit *test, unsigned long index,
				unsigned int order, unsigned int new_order)
{
	struct xarray *xa = xa_param(test);
...
static noinline void check_split(struct kunit *test)
{
	...
	for (order = 1; order < 2 * XA_CHUNK_SHIFT; order++) {
		for (new_order = 0; new_order < order; new_order++) {
			...
			check_split_2(test, 0, order, new_order);
			check_split_2(test, 1UL << order, order, new_order);
			check_split_2(test, 3UL << order, order, new_order);
		}
	}
}
#else
...
static const struct xarray *arrays[] = { &array };
KUNIT_ARRAY_PARAM(array, arrays, NULL);

static const struct xarray *xa0s[] = { &xa0 };
KUNIT_ARRAY_PARAM(xa0, xa0s, NULL);

static const struct workingset_testcase workingset_testcases[] = {
	{ &array, 0 },
	{ &array, 64 },
	{ &array, 4096 },
};

static void workingset_testcase_param_desc(const struct workingset_testcase *param,
					   char *desc)
{
	snprintf(desc, KUNIT_PARAM_DESC_SIZE, "index=%lu", param->index);
}

KUNIT_ARRAY_PARAM(workingset, workingset_testcases, workingset_testcase_param_desc);

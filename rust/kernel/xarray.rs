...
impl<'a, T: ForeignOwnable> Guard<'a, T> {
    ...
    pub fn remove(&mut self, index: usize) -> Option<T> {
        ...
        let ptr = unsafe { bindings::__xa_erase(self.xa.xa.get(), index) };
        // SAFETY:
    ...
    unsafe fn alloc(
        ...
        match unsafe {
            bindings::__xa_alloc(self.xa.xa.get(), &mut index, ptr, limit, gfp.as_raw())
        } {
            0 => Ok(to_usize(index)),
...
impl<T: ForeignOwnable> Reservation<'_, T> {
    ...
    unsafe fn replace(guard: &mut Guard<'_, T>, index: usize, ptr: *mut c_void) -> Result {
        ...
        // `XA_ZERO_ENTRY`.
        let ret = unsafe { bindings::__xa_cmpxchg_raw(guard.xa.xa.get(), index, old, ptr, 0) };
